...
public class Ghost extends MovingEntity {
	...
	public ArrayList<Vec2> findWayToVec2(Vec2 tile)	//A* algo https://de.wikipedia.org/wiki/A*-Algorithmus
	{
	...
	public void reset()	//setzt den Geist wieder an den Geisterstartpunkt zurück
	{
		setEatable(false);
	...
	public boolean isEatable()	//Gibt wieder, ob der Geist Essbar ist
	{
		return isEatable;
	}
	...
	public void changeSkin()	//Verändert die Textur für die Geister
	{
	...
	public void setMoves()	//Setzt den nächsten Zug für die Geister
	{
		...
		else
		{
			    	...
			    	if(path != null){
			    		if(path.get(1) == currentTile){path.remove(path.get(1));}
			    		if(path.get(1).isAbove(currentTile)) {setNextMoveDirection(2);}
			    			else if(path.get(1).isUnder(currentTile)) {setNextMoveDirection(0);}
			    			else if(path.get(1).isLeftTo(currentTile)) {setNextMoveDirection(1);}
			    			else if(path.get(1).isRightTo(currentTile)) {setNextMoveDirection(3);}
			    		if(getNextMoveDirection() == moveDirection) {path.remove(path.get(1));}
			    	}
			    	move();
